...
page = st.sidebar.radio("Перейти до розділу:", ["Аналіз та візуалізація", "Висновки"])

if page == "Аналіз та візуалізація":
    st.sidebar.divider()
    ...
    st.title("🐾 Аналіз даних притулку для тварин")
    st.markdown("""
    ...
    st.header("Висновки")
    st.subheader("1. Яка доля тварин найпоширеніша?")
    ...
    with col_dog:
        st.info("**Собаки (Dogs)**")
        st.markdown("""
        Для собак найчастішим сценарієм є усиновлення (41.7%). 
    
        Проте важливою відмінністю є високий показник повернення господарям (27.5%). 
        Це свідчить про те, що притулок ефективно працює як пункт пошуку загублених улюбленців.
        """)

    with col_cat:
        st.info("**Коти (Cats)**")
        st.markdown("""
        У котів інша динаміка. Майже половина тварин (49.4%) передаються іншому закладу.
    
        Це вказує на те, що для котів заклад діє як «транзитний хаб»: їх рідко забирають старі власники та не беруть 
        нові, тому основний потік перенаправляється до партнерів для подальшого догляду.
        """)

    st.info("""
    Загальний підсумок: хоча формально лідирує усиновлення (40.3%), стратегії роботи з видами кардинально різні: собак 
    частіше повертають додому, а котів - перенаправляють партнерам.
    """)

    st.subheader("2. Як вік впливає на шанси усиновлення?")
    st.markdown("""
    Аналіз підтверджує чітку тенденцію: чим молодша тварина, тим вищі її шанси знайти нову сім'ю.
    """)

    col_young, col_old = st.columns(2)

    with col_young:
        st.info("**Молоді (Junior & Young Adult)**")
        st.markdown("""
        Загальний підсумок: група з найвищим попитом на усиновлення.
        
        Собаки: усиновлення є безумовно лідеруючим результатом. На другому/третьому місці - передача іншому 
        закладу/повернення власнику. Інші сценарії зустрічаються рідко.
        
        Коти: мають специфіку в групі Junior. Тут лідирує передача партнерам, а на другому місці - усиновлення. 
        Це свідчить про використання програм перетримки для вирощування кошенят перед остаточним прилаштуванням.
        """)

    with col_old:
        st.info("**Дорослі (Adult & Senior)**")
        st.markdown("""
        Загальний підсумок: зниження ймовірності усиновлення. Пріоритет зміщується з пошуку нових родин на повернення 
        попередніх власників або передачу іншим партнерам.
        
        Собаки: спостерігається феномен заміщення - основним результатом стає повернення власнику. Більшість дорослих 
        собак ідентифікуються як загублені.
        
        Коти: дорослі коти мають значно менші шанси на усиновлення і частіше залишаються в системі притулків.
        """)

    st.subheader("3. Чи впливає стерилізація на результат?")
    st.markdown("Статус стерилізації є **критичним бар'єром.")

    col_fact1, col_fact2 = st.columns(2)

    with col_fact1:
        st.info("**Нестерилізовані/некастровані**")
        st.markdown(
            "Шанс на пряме усиновлення < 7%. Такі тварини скоріш за все передаються партнерам для операцій.")

    with col_fact2:
        st.info("**Стерилізовані/кастровані**")
        st.markdown("Основна категорія для усиновлення - ймовірність зростає до 48-67%.")

    st.divider()
    st.caption(f"Аналіз підготовано на основі датасету `pets.csv` | Виконала: студентка групи 8.04.F3.010.25.1 "
               f"Недашківська Валерія ")